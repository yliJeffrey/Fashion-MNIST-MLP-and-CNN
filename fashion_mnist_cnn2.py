...
# 0.24M parameters with random initialization
# 3 convolutional layers: 32 + 64 + 128 filters
# 1 fully connected layer: 128 units
# EarlyStopping(patience=20)
# loss: 0.1830 - accuracy: 0.9307 - val_loss: 0.2085 - val_accuracy: 0.9285 - 5s/epoch - 10ms/step
# batch_size=128, epochs=300
# best result obtained at epoch 66
...
def create_model():
    ...
    model.add(Conv2D(filters=128,
                     ...
                     padding='same',
                     activation='relu'))
    model.add(Dropout(0.5))    # 50% random dropout
    model.add(MaxPool2D(pool_size=(2,2)))
    ...
    plot_model(model, to_file='network/fmc2.png', show_shapes=True)
...
def train(model, batch_size, epochs, X_train, Y_train, X_test, Y_test):
    checkpoint = ModelCheckpoint('bestModel/fmc2.keras',
                                 monitor='val_loss',
                                 mode='min',
                                 save_best_only=True,
                                 verbose=1)
...
def main():
    ...
    model.load_weights("bestModel/fmc2.keras")
    print("\nsaved model to disk")
